...
این امتحان عملی ۳۰ تا ۴۰ دقیقه‌ای بر مفاهیم زیر تمرکز دارد: `class`, `object`, `constructor`, سطوح دسترسی (`public` / `protected` / `private`), مفهوم `abstract class`, پیاده‌سازی `interface`, ارث‌بری (`extends`) و خواندن داده با PDO. کل پروژه آماده است؛ فقط بخش‌های علامت‌خورده با عنوان «وظیفه (TODO)» باید توسط شما تکمیل شود تا یک فروشگاه کاملاً فارسی ساخته شود.
...
قبل از انجام وظایف (TODO) صفحه فقط یک پیام خالی نشان می‌دهد؛ بعد از تکمیل مراحل، کارت‌های محصولات فارسی به شکل کامل نمایان می‌شوند.
